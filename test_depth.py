...
def test_depth(opt):
    ##### load testing list #####
    test_files_list_path = '/ceph/data/carla_data/test_files/test_files_%s.txt' % opt.depth_test_split

    #Create python list of test files
    with open(test_files_list_path, 'r') as f:
        ...
        temp_test_files = [x.replace('_rgb', '') for x in test_files]

        sem_test_files = [sem_test_dir + t[:-5]+"_ss.npy" for t in temp_test_files]
