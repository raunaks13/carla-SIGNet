...
def read_file_data(files, data_root):
    """
    Extracts the date and image id from the filenames. Uses this to make velodyne point filenames
    gt_files: velodyne dir filename list
    gt_calib: calibration dir list
    im_sizes: image shape list
    im_files: image dir filename list
    Finally, returns all these lists to eval_depth_vis.py
    """
...
def generate_depth_map(calib_dir, velo_file_name, im_shape, cam=2, interp=False, vel_depth=False):
    ...
    return depth, depth_interp,velo_pts_im
